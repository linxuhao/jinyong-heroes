...
- **Changes:**
  ...
  5. **`_use_facility()`**: 
     ...
     - **Cost-gate:** `var cost = FacilityData.silver_cost(def)`; if `SaveManager.profile.silver < cost` → refuse (do NOT apply effects; render a "银两不足" refusal; `facility_use_count` does not increment). This is the natural reuse limiter (existing silver resource), not a new economy.
...
- **NEW debug action:** `debug_grant_silver` — bound to an unused physical key (implementer's choice; debug-only, not player-facing). Injects silver into the profile for the facility scenario's cost precondition (the `_fast_forward` cultivation path always picks practice/cultivate, never "work" — verified: `cultivation.gd::_fast_forward` L506-516 — so the player arrives at the map with 0 silver). `debug_grant_silver` is handled in `map.gd::_process` (same shape as `cultivation.gd::_process` debug actions): `SaveManager.profile.silver += <fixed amount>`. The amount is a single literal; it is NOT tuned — it just needs to be ≥ 2 × facility cost so the scenario can use the facility twice.
...
2. **`tests/test_map_data.gd`** [EDIT] — extend `_test_entry_content`:
   ...
   - `declared_gap_types("shaolin") == ["battle"]` (facility dropped), `declared_gap_types("wudang") == ["battle"]`, `declared_gap_types("luoyang") == ["battle", "facility"]` (unchanged).
   - The active-slot total is now 8 (5 events + 1 battle + 2 facilities), counted separately by type.
