...
**Net effect of the two §4.3 flips on this budget — both consequences:** §4.3 changes **two** intake streams, not one: the Shen Diao DR (enemy attacks on the player) *and* the West Poison reflect trigger (player attacks on West Poison). The net must count both sides.

- **DR side (enemy → player):** jade_flute_acupoint was post-DR 13 and is now **26 unreduced** (+13 per cast; cd3 → expect 2–3 casts over the fight under the §4.4 East policy, one of them inside the rounds 1–3 ramp). hidden_dragon was 62 unreduced and is now **31 post-DR** (−31 per cast; cd6 → at most one cast by round 8, a second only if the fight reaches round 12 — its cd6 also keeps it out of the rounds 1–3 ramp, so it cannot affect the >354 gate). DR-side net: **+26..+39 − 31..62 = −36..+8**.
- **Reflect side (player → West Poison):** both of Yang Guo's external arts are melee classes (玄铁剑法 = 剑, 黯然销魂掌 = 拳掌), so under §4.3 **100% of the player's attacks are now melee-class**, and 蛤蟆反震 (12 × 1.3 = **16**) fires on every one of them that lands on West Poison. Under the old distance heuristic the non-adjacent hits resolved as ranged and the reflect stayed silent: Great Craft (line 3, cd2 → ~4–5 casts per fight), Cleave (cross 2, cd3 → ~3 casts, outer ring), Boundless Seas (self radius 2, cd6 → ~2 casts, outer ring). Adjacent-only attacks (basic, Heavy Edge, Heart Strike, Mud Drag, Wandering Valley's landing AoE, 17 Forms) were melee under both rules — no change there. **Assumed hit count: 3–6 of those non-adjacent hits catch West Poison** over the fight ⇒ **+48..+96 intake** (T4 must check this assumption against the measured `CombatManager.debug_reflect_hits` trace, §7.2; it is structurally bounded too — West Poison dies with the melee cluster in rounds 4–7, so reflect stops when he does).
- **Total net: (−36..+8) + (48..96) = +12..+104 — meaningfully hotter, not cooler.** The envelope bounds themselves are design-derived and do **not move**: **508–633 for R=8, 612–737 for R=12, steering target ≈560–685 for R=10**. What moves is the baseline, and each bound moves its own way: the added heat **helps the >354-by-round-4 gate** (the early Flute cast adds +13, and Boundless Seas can first catch West Poison from its outer ring in round 2) but **tightens the death bound** (cumulative intake < 500 + 26r at every r loses up to ~96 of margin). Plainly: at 5–6 reflect hits the baseline lands **above** the band, and even at 3–4 it sits in the band's upper half — the §4.4 compensation below (West Poison dies first, clip-West-only-when-efficient casts, stronger ranged throttle if hot) is what brings it back in. If the measured trace still misses the band after that, tune the §4.4 predicates further — the classification itself is not a balance knob.
...
**Reflect compensation (player → West Poison, from the §4.2 net):** the corrected net is hot, so reflect intake must stay at the low end of the §4.2 assumption — **3–4 non-adjacent reflect hits (≈ +48..+64)** over the fight. Levers, in order: (1) **West Poison dies first among the melee trio** — his 115 HP is the trio's softest (North's Iron Bones −15% makes his effective HP higher, and Central's innate-qi guard would waste a lethal blow, so Central is last); West's existing approach-and-hold keeps him inside the player's reach, and the player's single-target cleanup concentrates on him from round 2 onward — reflect intake dies when he does (rounds 4–7). (2) **Multi-target casts are aimed so that clipping West Poison is accepted only when the cast also hits ≥ 2 other targets** — never gratuitously (Great Craft's line prefers the Central/North axis when the damage tradeoff is equal; Cleave / Boundless Seas center on the densest non-West cluster). (3) If the trace is still hot, **tighten the ranged-pair throttle first** (East's even-round basic gate drops to every 3rd round; South's Six-Pulse waits until its target is below 50% HP) — never the melee trio's adjacency engagement, whose 12–24 post-DR hits are the design's affordable baseline. T4 verifies the 3–4-hit assumption against the measured `CombatManager.debug_reflect_hits` trace (§7.2).
...
CombatManager: [current_round, phase, active_unit_name, turn_order, turn_log,
                ...
                debug_await_frames, debug_round_frame, debug_reflect_hits]   # + debug_round_frame (new: frame at which _begin_round ran); + debug_reflect_hits (new: cumulative 蛤蟆反震 trigger count — the observable for the T3/T4 reflect-side checks)
...
`debug_round_frame` (int, `Engine.get_process_frames()` in `_begin_round`) exists purely to let the PM re-time press frames against measured round boundaries. `debug_reflect_hits` (int, incremented each time 蛤蟆反震 triggers) lets T3/T4 verify the §4.2 reflect-side hit-count assumption directly.
...
3. **T3** — Melee/ranged classification (C1, §4.3). Gate: protected scenarios still green; terminal run still deterministic; and the two flipped skills verified specifically — a jade_flute_acupoint hit on the player is **NOT** reduced by Shen Diao DR (intake 26, not 13), and a hidden_dragon hit **IS** reduced (intake 31, not 62); and a Great Craft cast on West Poison from distance > 1 **MUST** trigger 蛤蟆反震 (player takes **16**, observable via `debug_reflect_hits`) — the reflect side of the same flip.
