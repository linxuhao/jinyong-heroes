...
| cultivation | `GONGFA_PICK` (**empty**) | single `CultOptionButton0` 「度过本月」(label SUPERSEDED 2026-09-01 `jinyong-loop` R2,原「返回行动」) `cultivation.gd:572-576`; pressed → `_on_option_pressed` → `_on_accept` empty branch → `phase = "ATTR_PICK"` + `_after_action()` **月份照常推进**(exit behavior SUPERSEDED 2026-09-01: 原静默返回 ACTION_PICK、月份不动的死端被月度推进取代——`90_decisions.md` 2026-09-01 条 (a)); empty hint states the exit `:833-838` | **Y** | was N (**the P0 dead-end** — zero buttons, `box.visible = not labels.is_empty()` hid everything, keyboard-only exit) | **FIXED this round**: always-one-button guarantee (`cultivation.gd:542-550` comment documents it); phase-diff pipe pinned by `playtest/clicks_only_gongfa_empty_exit.yaml` |
