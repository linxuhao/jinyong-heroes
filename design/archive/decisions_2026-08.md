# Decisions Archive — 2026-08 (moved verbatim from design/90_decisions.md)

> Source: `design/90_decisions.md` (2026-09-03 ledger slimming, Card N+2 / C7.1).
> The sections below are superseded or already-landed long-form derivations, moved
> **verbatim** (byte-identical to the source at move time) so the current file can be
> a one-line-per-decision table. The current `design/90_decisions.md` table carries
> one index pointer row per moved decision. Per-month naming: future slimming rounds
> land their own `decisions_YYYY-MM.md` here.
