...
| UX-36 | **OPEN** — record-only(同上;由后续队列「外号」项解决) | 养成 / 角色面板 / 战斗 | `ProgressionHero` / `Sparring Partner` 英文裸 id 原样上屏 | 主角与陪练没有可读名号;外号(昵称)轮落地时一并解决 |
| UX-37 | **OPEN** — R3b 官方运行红(2026-09-02 官方 93 场景整合运行 `playtest_summary.md`;修法 = boot 时序重基线 + 表达式书写修正,零断言放松)。**本轮动作已落**:`fix_scenario_boot_rebaseline`(五条场景 boot/相位时序重基线 + 表达式书写修正)与 `fix_gate_i18n_ending_copy`(ratio guard regex ↔ canonical assert-grammar 同步)已落地;**CLOSED(R3b numbers-bind)** — post-fix 官方复跑证据已落(2026-09-02,修复后树官方 93 场景运行:硬闸门 `passed: true`、`spec_used: true`、180 帧、零 runtime error)实测五条全 PASS:`practice_target_receipt` **43/43**、`ending_tiers_differentiate` **27/27**、`work_beats_idling` **26/26**、`ending_divergent_playstyles` **33/33**、`ending_last_month_choice` **38/38** | playtest 契约层(R3b 新钉 + 结局家族既有钉) | 五条场景时间线停 TUTORIAL / 相位逐月错位(boot 就绪时序与 `at:` 帧不匹配):`work_beats_idling` **11/21**(TUTORIAL 态点 `CultOptionButton0/2` → 节点不存在,构成本轮 85 条 runtime error 的主体)、`ending_tiers_differentiate` **13/22**(f720 期望 CULTIVATION 实测 TUTORIAL;`ending_tier_history[1]` 索引错位系第二条结局未到达)、`practice_target_receipt` **29/40**(f410 相位错位致点击落到别的选项;表达式把 `text.contains("罗汉拳")` 写进引号内 → 恒假标量比较,重蹈 2026-08-23「值里必须含比较/逻辑运算符」教训)、`ending_divergent_playstyles` **19/27** / `ending_last_month_choice` **16/30**(既有钉,f925/f320 停 TUTORIAL、month 实测 10≠36) | C2 零差分、C3 tier 差分、C7 比率钉、结局家族差分的**官方屏上证明未建立**——观测数据不指向玩法代码回归,但验收欠着;修复后须官方复跑收口。**已收口**:修法 = `fix_scenario_boot_rebaseline`(boot/相位时序重基线 + 表达式书写修正)+ `fix_gate_i18n_ending_copy`(ratio guard regex 同步),零断言放松;post-fix 官方复跑五条全 PASS(证据 = `playtest_summary.md: practice_target_receipt 43/43 / ending_tiers_differentiate 27/27 / work_beats_idling 26/26 / ending_divergent_playstyles 33/33 / ending_last_month_choice 38/38`),C2 零差分、C3 tier 差分、C7 比率钉、结局家族差分的官方屏上证明自此建立 |
| UX-38 | **OPEN** — R3b 官方运行红(同上;重基线一次,零断言放松)。**本轮动作已落**:`fix_readiness_verdict_rebaseline` 按带/钉测量 STOP-and-report——fresh-boot power 45 vs M3' balanced 40,冲突 `even≥46 vs even≤40` 不可满足,带与 f260 字面均未改,冲突已上报;**CLOSED(R3b numbers-bind)** — 带冲突其后由所有者带裁决解决(r3c_readiness_bands_fresh_max:P_fresh_max = 60、even = 61 = P_fresh_max + 1、strong = 124,见 `40_progression.md` M3''' 表),f260 `战备不足` 字面逐字成立,post-fix 官方复跑(2026-09-02)实测 `huashan_readiness_warning` **16/16** PASS | 角色页 · 华山评估 | `huashan_readiness_warning` **20/21**:f260 字面钉 `readiness_text == "华山评估：战备不足"` 实测 `华山评估：势均力敌`——重基线对该真实档 boot 的档位猜错一档(该 boot 的 readiness power ≥ 38,落 even 带);f350 STRING 差分钉仍绿 | 预警词与该 boot 的实际档位差一档;按 `40_progression.md` M3' 表重推导该 boot 的 power 后重基线字面(词表/格式不动,只换档位)。**已收口**:按带裁决重推导后 f260 字面逐字成立(带改动记录于 `40_progression.md` M3''' 表与 `90_decisions.md` R3c 带裁决),post-fix 官方复跑 **16/16**(证据 = `playtest_summary.md: huashan_readiness_warning 16/16`),词表/格式零改动、零断言放松 |
| UX-39 | **OPEN** — R3b 官方运行红(同上;数字即契约 → 重新推导,不是放松)。**本轮动作已落**:`cultivation_changes_combat` 已转性质钉(owner 裁决:按性质重推,非字面量;×0.85→×1.1、34→27 为转换前字面钉观测,sidecar 30/30);`action_yield_differential` 根因经 `fix_scenario_boot_rebaseline` 处理(boot 时序重基线);**CLOSED(R3b numbers-bind)** — post-fix 官方复跑(2026-09-02)实测 `cultivation_changes_combat` **30/30**、`action_yield_differential` **44/44** 全 PASS | 战斗 / 养成既有回归网 | `cultivation_changes_combat` **27/30**(C1 mastery 项进入 `derive_stats` 的真实后果:发挥期望 ×0.85 → 实测 **×1.1**、Sparring_Partner 34 → **27**——场景钉的就是遭遇战数值,按 `00_roadmap.md`「数字本身就是契约」例外条款重新推导,不许不带因改绿)、`action_yield_differential` **27/38**(f280 `last_action_kind` 期望 practice 实测 work、`last_practice_amount` 实测 0、相位逐月错位——根因待诊断,先查 C2 target 透传落地后 GONGFA_PICK 选项序/月份是否位移) | 既有回归网两条红:一条是 C1 数值落地的可预期后果(重推导),一条根因未明(诊断后定修法);两者收口前「既有场景零回归」不成立。**已收口**:post-fix 官方复跑两条全 PASS(证据 = `playtest_summary.md: cultivation_changes_combat 30/30 / action_yield_differential 44/44`),「既有场景零回归」在修复后树上重建 |
