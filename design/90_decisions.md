...
| 2026-09-04 | 固定帧路线钉退役(所有者裁决,反馈 #6) | pre-R5 时代的两条固定帧路线钉 `huashan_winnable_normal_route` 与 `ending_last_month_choice` **退役**(yaml + 两处注册表行删除):它们钉的是一条约 2200 帧的固定点击 PATH,月份语法已被 R3、R3b、R5 三次重推导,任何上游屏改动(焦点顺序、返回按钮、确认闸)都会挪动里程碑帧,维护成本与脆弱性不再匹配其覆盖;同族 `cultivation_year_end_stay` 与 `sect_switch_same_school_connects` **原位重锚**(insert-only 变更表、断言块逐字节不变)各 8/8;多屏遮挡主网 `consequence_screens_occlusion.yaml` 随路线钉一并退役(所有者许可的范围变更),遮挡覆盖 = map 腿 `consequence_screens_occlusion_map` + 各屏钉 + `occlusion_no_button_over_text`。裁决只撤脆弱载体,不撤被钉的性质——结局可达 / 年末可停留由重锚钉与其余 116 条场景网承载 | `playtest_summary.md`(2026-09-04 post-fix 终局官方 118 场景,0 runtime error,0/118 断言失败):`cultivation_year_end_stay` 8/8、`sect_switch_same_school_connects` 8/8、`occlusion_no_button_over_text` 22/22、`consequence_screens_occlusion_map` 9/9;`final/delivery_notes_fix_route_timeline_anchors.md` §5/§6;`tests/test_ending_gate_pins.py` 退役条目日期化 carve-out |
| 2026-09-04 | route 型帧脚本场景退役(所有者裁定) | max `at:` ≥ 800 的整局帧脚本场景退役;单元场景(≤200 帧、单屏单态、直接引导)保留并承接其属性;5_vision 六问闸门不变;三条逐字闸门(facility_use_reusable / map_node_event_shaolin / map_battle_node_huashan)+ event_travel_effects 字节一致 + RNG 生命线豁免;承接不了的整流程/难度属性由 per-round 真浏览器试玩承载;无法廉价承接者保留待裁 | `playtest/RETIRED.md`;`99_changelog.md` route-retirement 行 |
...
- **2026-09-04 · 固定帧路线钉的持久形态 —— 已由所有者裁决(反馈 #6,2026-09-04),提问结清,本节自此无待决问题。**
  裁决:两条 pre-R5 固定帧路线钉 `huashan_winnable_normal_route` / `ending_last_month_choice` 退役、同族 `cultivation_year_end_stay` / `sect_switch_same_school_connects` 原位重锚(insert-only、断言逐字节)、多屏遮挡主网 `consequence_screens_occlusion.yaml` 随路线钉一并退役——原提案 (a)「节点可见时点击」与 (b)「存档 boot」均不再采纳为载体。现行规则与证据见上方 2026-09-04「固定帧路线钉退役」行;提问原文(提案 (a)/(b) 推导)与 119 场景实测记录保留于 git 历史与 `final/delivery_notes_fix_r5_huashan_route_drift.md`。
