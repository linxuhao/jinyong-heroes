...
这是**一个武学全满的独臂大虾**(本作没有人物等级,只有武学等级),不是唯一的独臂大虾。全部功法发挥度 **1.3(超常)**,
前置视为已满(教程战不需要玩家去练)。
...
|---|---|---|---|---|---|---|
| 东邪虾 | 95 | 4 | 85 | 22 / 3 | 碧海潮生功(甲·阴) | 碧海潮生曲(甲·阴,**乐器**) |
| 西毒虾 | 115 | 3 | 70 | 26 / 1 | 蛤蟆功(甲·刚) | 灵蛇拳(甲·刚,**拳掌**) |
| 南帝虾 | 100 | 3 | 76 | 24 / 2 | 先天功(甲·柔) | 一阳指(甲·阳,**指**) |
| 北丐虾 | 120 | 3 | 74 | 28 / 1 | 混天功(甲·阳) | 降龙二十一掌(甲·阳,**拳掌**) |
| 中神通虾 | 130 | 3 | 80 | 26 / 1 | 先天功·全真(甲·阳) | 全真剑法(甲·阳,**剑**) |
...
> **内力池本轮只存不耗。** 内功产出的内力值(独臂大虾 180)存在数据里、显示在界面上,
...
`max_health = 135`——不是教程独臂大虾的 1000;预授权的 §D3 降阵容预案(首回合全局伤害
...
<!-- nickname-ruling-record -->
## 附:R4 外号裁定记录(2026-09-03,record-only)
...
本轮新造的外号(按 owner 的「刀贩虾」=身份名词+虾 规则,用于原无人名的路人):

| 旧屏上串 | 新外号 | 说明 |
|---|---|---|
| 侠客 / 裸 `ProgressionHero` | 侠客虾 | 主角位(map/encounter 战);此前裸英文键上屏(pt2 帧) |
| 陪练弟子 / 裸 `Sparring Partner` | 陪练虾 | 切磋遭遇;此前裸英文键上屏(pt3 帧) |

EN 译名(本轮 coined,非 owner 逐字锁;仅 zh 侧受 owner 约束):
独臂大虾=One-Armed Prawn;东邪虾=East Heretic Shrimp;西毒虾=West Poison Shrimp;
南帝虾=South Emperor Shrimp;北丐虾=North Beggar Shrimp;中神通虾=Central Divine Shrimp;
侠客虾=Wanderer Shrimp;陪练虾=Sparring Shrimp。

内部键(不动):character_name 值 `Yang Guo`/`East Heretic`/…/`ProgressionHero`/
`Sparring Partner`、节点名 `East_Heretic`…、`turn_order` token —— 三处逐字保护的
闸门靠它们,外号只改显示层。
<!-- nickname-ruling-record-end -->
