...
**R5 反转(2026-09-04 `jinyong-nav`,所有者 2026-09-02 试玩裁定;取代上段,上段存史)。**
R2 用「烧月」换绿的裁决被推翻:软锁的定义是「玩家没有任何路可走」,而 R2 的空列表
出口恰恰是「有路可走但烧掉一个月」。现行规则:无可练功法时该次确认**返回 ACTION_PICK
重选**,月份/银两零差分、零 RNG,状态文案「功法均已大成,已返回行动重选」,空态按钮
标签回**「返回行动」**(返回按钮不得自称「度过本月」);三颗钉重推导为返回+零差分
(`softlock_empty_practice_returns` 换名 + 两条同文件重推,逐行变更表在各 yaml 头,
烧月断言零保留),官方实测 16/16 · 19/19 · 16/16。M2' 腿 A do-nothing 不再经该按钮
(见 §9 R5 注记);2026-09-04 官方树两条集成红(F1 :1108 两参 get / F4 两按拜师×逐字闸门)
修复后收口,不属本规则。
...
- **腿 A · do_nothing**:空档 `度过本月` ×36(键盘语法)。该按钮只在「无未大成
  ...
  **R5 注记(2026-09-04)**:腿 A 的载体按钮已不存在——空功法列表的确认现在**返回行动**
  (月份零差分,见 §3 R5 反转注记);本表 5 种子的腿 A 数字是 R3b 测量时的历史记录,现行
  do-nothing 路线由腿 A'(真实档 + 36 × 收卡)承载,若下一轮需要重测腿 A,改走 36 × 修习腿
  并按「数字即契约」重推导,不删钉。
