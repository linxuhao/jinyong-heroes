...
> ⚠️ **Verification status: NOT deployable - the vision gate failed Q5.** The three jinyong-events deliverables (the eight-layer portrait-visibility predicate + the 92 px clamp fix, the 16-row travel-event pool, the no-repeat bag observable + its new scenario) are landed and wired end-to-end with a measured pre-fix probe record (`final/portrait_cover_probe_notes.md`). The downstream gates have now run (as read by the `5_review` verdict from the gate reports): compile clean, pytest 10 passed, playtest hard gate passed with 46/47 green (only the deliberately-red `terminal_victory`), `portrait_visibility` 22/22, `event_travel_effects` 19/19 - **but the vision gate answered NO on Q5 (health bars recognisable) in 19/28 battle scenarios**, and no green re-run exists. Q5 is the single blocking item. See [Verification Status](#verification-status).
...
**Runtime gates have run - one hard failure (vision Q5) blocks the round.** The gate results below are the `5_review` verdict's readings of the gate reports (`compile_report.json` / `test_report.json` / `playtest_summary.md` / `vision_report.json`); those report files are not persisted in the repo workspace, so nothing here is independently re-read. Unproduced or unreadable evidence is never reported as passed - the Q5 failure is carried forward as the blocking item, not buried.
...
- **Vision gate (`5_vision`): FAILED - Q5.** Q5 (health bars recognisable: a filled portion AND an empty portion showing remaining HP) answered **NO in 19/28 battle scenarios** ("health bars appear full, no empty portion"), run on the fallback vision model. This is the round's single blocking item - see Known Issues below.
...
- **Vision Q5 (health bars recognisable) FAILED and is unresolved - the round's blocking item.** The post-review probe (`final/health_bar_q5_probe_notes.md`) pinned 13 frames (8 full-HP + 5 injured, HP verified via playtest observables, ≥ 5 distinct scenarios per group) but could not classify the failure: the primary-model `/vision` run is produced only by the downstream `5_vision` gate (no `vision_report.json` on disk at probe time), so every model answer is recorded **PENDING** and the gated fix changed **no code** (`EMPTY_CAP_PX = 14.0`, expand margin 8.0, `EmptyCap` rect all verified unchanged). The three-case decision - (1) the primary model also says NO on injured frames = real defect; (2) only full-HP frames fail = Q5 applicability (the cap design flattens the 78–100% band, so a full-HP bar renders ~78–82% filled); (3) the primary model is all green = fallback-model limitation - is deferred to a real primary-model run. **Q5 must be green on a fresh gate run before this round can pass; do not mark the round deployable until then.**
