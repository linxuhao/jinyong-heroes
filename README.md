...
The 16 rows and their trade-off shapes are also recorded in the design archive: `design/20_content.md` §4「游历事件池 (16 条)」 (content-shape record; `EventData.TABLE` stays the authoritative source).
...
- Post-`5_review` doc closures verified: `design/20_content.md` §4「游历事件池 (16 条)」 records the 16-row pool by trade-off shape (the review's recorded debt 1, resolved); `design/30_presentation.md` carries the 2026-08-26 Q5 status note; `design/99_changelog.md` carries the jinyong-events row plus the 2026-08-26 wording-alignment row; `design/90_decisions.md` records the rejected gate-side-question-rewrite decision.
...
- **Vision gate (`5_vision`): no green verdict produced.** Per the `5_review` verdict's reading of the `vision_report.json` gate artifact (not persisted in the repo workspace): `blind:true, endpoint_unreachable:true` - the primary vision model endpoint was unreachable at gate time and the fallback failed mid-run after 4 calls (IncompleteRead). Only 4/47 scenarios were judged, all full-HP battle frames; the 4 Q5=NO answers are consistent with the documented 78–100% HP flattening design. Zero injured frames were judged. The next `5_vision` gate run - on a reachable primary model - must produce the real verdict and resolve Q5's three-case decision; no gate-side fix is claimed (none is on record). No code change to `health_bar.gd` was made pending that verdict.
...
1. ~~**`design/20_content.md` has no section for the 16-event pool**~~ **Resolved** (post-`5_review` doc-closure round, 2026-08-26, recorded in `design/99_changelog.md`): `design/20_content.md` §4「游历事件池 (16 条)」 now records all 16 rows grouped by trade-off shape (item-with-cost / item-vs-item / item-vs-attr / attr-A-vs-attr-B / growth-vs-money / paid-growth / paid-attr-vs-free-attr / pay-vs-effort / money-vs-attr / moral fortune-vs-silver / charity), with `scripts/data/event_data.gd`'s `EventData.TABLE` as the authoritative source.
